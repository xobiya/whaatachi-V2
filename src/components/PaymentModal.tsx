...
export default function PaymentModal({
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (userGender === 'Male') {
      ...
      setTimeout(() => {
        onSubmitPayment(profile.id, profile.name, profile.image, currentUser?.name || 'Male Member', currentUser?.contactInfo?.phone || 'Auto', autoTxId, method, 200, uploadedFileData || undefined);
        setSubmitting(false);
        onPaymentSuccess();
      }, 800);
    } else {
