  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (text.includes('cbe') || text.includes('telebirr') || text.includes('payment') || text.includes('fee')) {
        botText = "Got you! If your Telebirr or CBE receipt is pending approval, paste the Transaction ID on the payment unlock card or check the Admin Queue. Double-check you sent exactly 200 Birr.";
      } else if (text.includes('unlock') || text.includes('contact')) {
        botText = "To unlock a match: (1) Find your match on the Discovery dashboard, (2) Hit 'Unlock Contact', (3) Transfer 200 Birr to the merchant accounts shown, (4) Paste the Transaction ID. You'll see their contact info in 'Unlocked History' upon validation.";
      } else if (text.includes('free') || text.includes('women') || text.includes('price')) {
        botText = "Whaatachi is 100% FREE for women who verify their identity. Men pay a one-time 200 Birr fee to maintain a premium, spam-free space.";
      }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          ...
          <div className="md:col-span-2 bg-white dark:bg-[#1A1118] border border-[#EDE6D9] dark:border-[#C9A84C]/10 rounded-3xl overflow-hidden flex flex-col h-[500px] shadow-sm transition-colors">
            ...
            <div className="bg-[#8B0020] px-5 py-4 text-white flex items-center justify-between shrink-0">
                ...
                return (
                  <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-fade-in text-xs`}>
                    <div className={`max-w-[80%] rounded-2xl p-3.5 leading-relaxed tracking-wide ${
                      isUser
                        ? 'bg-[#8B0020] text-white rounded-br-none shadow-sm'
                      ...
                      <span className={`block text-[9px] mt-1.5 text-right font-light ${isUser ? 'text-[#F0D4D4]' : 'text-gray-400 dark:text-gray-500'}`}>
