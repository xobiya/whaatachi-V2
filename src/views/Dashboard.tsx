    ...
    <div className="space-y-5">
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <select value={selectedCity} onChange={(e) => setSelectedCity(e.target.value)} className="w-full pl-9 pr-4 py-3 border border-[#EDE6D9] dark:border-[#C9A84C]/15 rounded-xl bg-white dark:bg-[#1A1118] text-sm text-gray-800 dark:text-[#FFFCF8] focus:outline-hidden focus:border-[#8B0020] dark:focus:border-[#C9A84C] appearance-none">
        ...
        {showFilters && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-[#1A1118] rounded-t-3xl max-h-[85vh] overflow-y-auto p-6 pb-8 animate-slide-up shadow-2xl">
              ...
              {filtersContent}
              <button onClick={() => setShowFilters(false)} className="w-full mt-5 py-3.5 bg-[#8B0020] text-white font-bold text-sm rounded-xl cursor-pointer">
                Apply Filters
              </button>
            </div>
          </div>
        ...
        <div className="flex gap-8 items-start">
          ...
          <div className="flex-1 min-w-0 space-y-5">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
              <button onClick={() => setFilterType('all')} className={`px-4 py-2.5 rounded-xl text-xs font-bold shrink-0 transition-all cursor-pointer border ${filterType === 'all' ? 'bg-[#1A1118] dark:bg-[#8B0020] border-[#1A1118] dark:border-[#8B0020] text-white' : 'bg-white dark:bg-[#1A1118] border-[#EDE6D9] dark:border-[#C9A84C]/15 text-gray-600 dark:text-gray-400 hover:border-gray-300'}`}>
                All ({profiles.length})
              </button>
              <button onClick={() => setFilterType('recent')} className={`px-4 py-2.5 rounded-xl text-xs font-bold shrink-0 transition-all cursor-pointer border ${filterType === 'recent' ? 'bg-[#1A1118] dark:bg-[#8B0020] border-[#1A1118] dark:border-[#8B0020] text-white' : 'bg-white dark:bg-[#1A1118] border-[#EDE6D9] dark:border-[#C9A84C]/15 text-gray-600 dark:text-gray-400 hover:border-gray-300'}`}>
                Active
              </button>
              <button onClick={() => setFilterType('verified')} className={`px-4 py-2.5 rounded-xl text-xs font-bold shrink-0 transition-all cursor-pointer border ${filterType === 'verified' ? 'bg-[#1A1118] dark:bg-[#8B0020] border-[#1A1118] dark:border-[#8B0020] text-white' : 'bg-white dark:bg-[#1A1118] border-[#EDE6D9] dark:border-[#C9A84C]/15 text-gray-600 dark:text-gray-400 hover:border-gray-300'}`}>
                Verified
              </button>
            ...
            <div className="bg-[#F8F4ED] dark:bg-[#1A1118] border border-[#EDE6D9] dark:border-[#C9A84C]/10 rounded-xl p-3 text-gray-700 dark:text-gray-300 text-xs leading-relaxed flex items-start gap-2.5">
              ...
              <div>
                {isLoggedIn ? (
                  <><span className="font-bold">Viewing as {userGender}.</span> {userGender === 'Male' ? 'Men pay 200 ETB per unlock. Women get free access.' : 'Women unlock contacts for free. No payment needed.'}</>
                ) : (
            ...
            {filteredProfiles.length > 0 ? (
              ...
              <div className="bg-white dark:bg-[#1A1118] border border-[#EDE6D9] dark:border-[#C9A84C]/10 rounded-2xl py-16 px-4 text-center max-w-xl mx-auto space-y-3">
                ...
                <button onClick={handleResetFilters} className="px-4 py-2.5 bg-[#8B0020] hover:bg-[#B31B3A] text-white font-semibold text-xs rounded-lg shadow-sm cursor-pointer transition-all">Show All Profiles</button>
              </div>
            )}
          </div>
        </div>
